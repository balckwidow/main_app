...
const PenList = () => {
  ...
  useEffect(() => {
    setPageContent(data.slice(0, 4));
  }, []);

  const handleChange = (event, value) => {
    setPage(value);
    setPageContent(data.slice(value + 2, value + 5));
  };
    ...
    <>
      ...
      <div className="mt-1 bg-white w-full">
        <div className="mx-4 ">
          ...
          <div
            className="mt-6 grid grid-cols-2 w-full gap-y-4 justify-items-center
                  ...
                  <Link to={"/pen/" + val.name} key={index}>
                    <div className="group  w-fit h-fit  p-3">
                      ...
                      <div className="mt-2 flex items-center flex-col w-full">
                        ...
                        <p className="text-sm  text-gray-900 sm:text-base lg:text-lg">
                          LBP {val.price}
                        </p>
                      </div>
                      <div className="flex justify-center w-full mt-2 gap-2 sm:gap-4">
                        {val.color.map((v) => {
                          return (
                            ...
                            rounded-full sm:h-5 sm:w-5 lg:h-6 lg:w-6 xl:w-7 xl:h-7`}
                              key={val.name + "_" + v}
                            ></div>
                          );
                        })}
